...
> 11. In GitHub-connected projects (as defined in `## 0`), after validation and before completion, run non-destructive delivery steps (`git add` → `git commit` → `git push`) unless explicitly prohibited by the user or blocked by a real error
---
...
2. **If something is unclear, ask or state assumptions.**
   ...
   - If multiple reasonable interpretations exist, the agent must list them briefly and then either:
      - ask which to follow (only if the choice affects output/behavior), or
      - choose one explicitly and label it as an Assumption (and explain why it was chosen).
   - If a simpler approach exists that still satisfies the explicit request, the agent must propose it.
     Push back when warranted (but do not silently reduce scope).
...
1. **Stay within the requested scope.**
   ...
   - Prefer the minimum code/change-set that solves the requested problem.
   - Do not add speculative flexibility/configurability/abstractions that were not requested.
...
3. **Be explicit about side effects.**
   ...
   - When editing existing code: make surgical changes only.
      - Do not refactor or reformat adjacent code unless required by the request.
      - Match the existing style/conventions.
      - Remove only the unused imports/vars/functions made unused by YOUR changes.
...
2) Run a **pre-research step** and record:
   ...
   - Options: if multiple interpretations exist, list them; do not pick silently
   - Tradeoff: if a simpler approach exists, note it (and whether it changes scope)
3) Perform a **deep analysis step** before edits:
   ...
   - Complexity check: confirm the chosen approach is the simplest one that meets explicit requirements
4) Execute implementation **strictly in TODO order** without skipping.
   ...
   - Implementation constraints:
      - Simplicity first: no speculative features, no one-off abstractions, no premature configurability
      - Surgical diffs: touch only what is necessary; avoid unrelated cleanup
      - Clean up only the orphans your changes create (e.g., unused imports/vars)
5) Run a **review step** before completion (explicit self-review required).
   - Review checklist (minimum):
      - Every changed line traces back to the user request and a TODO item
      - No unnecessary complexity ("Would a senior engineer call this overcomplicated?")
...
## 10. Coding Quality Guardrails (Merged from CLAUDE.md)

These guardrails refine *how* to execute the required workflow in §8. They bias toward correctness and minimal diffs over speed.

### 10.1 Think Before Coding
- Do not assume away ambiguity. Surface it.
- If uncertain: ask only when the ambiguity is blocking or would materially change behavior/output.
- If proceeding with an inference: state it explicitly as **Assumption:** and continue.
- If multiple interpretations exist: list them; do not pick silently.
- If a simpler approach exists or the request seems over-scoped: propose it and push back when warranted.

### 10.2 Simplicity First
- Implement the minimum code that satisfies the explicit request.
- No speculative features, configurability, or “future-proofing” unless requested.
...
- If your change creates unused imports/vars/functions, remove those.
- Do not remove pre-existing dead code unless explicitly requested; mention it instead.

### 10.4 Goal-Driven Execution
- Turn vague tasks into verifiable goals with explicit success criteria.
- Prefer tests to validate behavior when feasible:
   - Bug fix: reproduce with a test, then make it pass.
   - Validation feature: add failing tests for invalid inputs, then make them pass.
- For multi-step tasks: define a brief plan where each step has a verification target.